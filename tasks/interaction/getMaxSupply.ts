...
task("interaction:NFTgetMaxSupply").setAction(async function (_, { ethers }) {
  ...
  try {
    const res1 = await nft.maxSupply();
    console.log(`NFT: max supply is ${res1} `);
  } catch (e) {
    console.error(" NFT error max supply", e);
  }
});
